...
export const getTaskStatus = async (taskId) => {
  try {
    ...
    const response = await fetch(`${MESHY_API_BASE_URL}/openapi/v1/image-to-3d/${taskId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${MESHY_API_KEY}`,
      },
...
export const refineModel = async (taskId) => {
  try {
    ...
    const response = await fetch(`${MESHY_API_BASE_URL}/openapi/v1/image-to-3d/${taskId}/refine`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${MESHY_API_KEY}`,
        'Content-Type': 'application/json',
