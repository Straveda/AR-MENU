...
export default function DishDetails() {
  ...
  const fetchDish = async () => {
    try {
      ...
      try {
        const recData = await axiosClient.get(`/dishes/${id}/also-ordered`);
  ...
  return (
    <div className="min-h-screen bg-amber-50">
      {/* Header */}
      <div className="border-b border-amber-200 bg-white sticky top-0 z-20 shadow-sm">
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8">
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
              ...
              <div className="bg-white rounded-2xl p-1 shadow-sm border border-amber-50">
                {arStatus.color === "green" ? (
                  <button
                    ...
                    className="w-full group bg-linear-to-r from-gray-900 to-gray-800 hover:from-black hover:to-gray-900 text-white py-4 px-6 rounded-xl font-bold shadow-lg transition-all flex items-center justify-between transform hover:-translate-y-0.5"
                  >
          ...
          <div className="w-full lg:w-1/2 space-y-8 pb-20 lg:pb-0">
            ...
            <div className={`lg:hidden mb-6 pb-6 border-b border-gray-100 ${!dish.available ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className="flex flex-col gap-4">
                ...
                <button
                  ...
                  className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold text-lg py-4 px-8 rounded-xl shadow-xl shadow-amber-200 hover:shadow-2xl transition-all transform hover:-translate-y-1 flex items-center justify-center gap-3"
                >
                  <span>Add to Order</span>
                  <span className="bg-amber-700/50 px-2 py-0.5 rounded text-sm">₹{dish.price * quantity}</span>
                </button>
                ...
                <div className="text-center">
                  <button onClick={() => navigate('/cart')} className="text-sm font-semibold text-gray-500 hover:text-amber-600 underline decoration-2 decoration-transparent hover:decoration-amber-200 transition-all">
                    View current order →
                  </button>
                </div>
              </div>
                    ...
                    {recommendations.map((recDish) => (
                      <div key={recDish._id} className="w-40 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden shrink-0">
                        <div 
                          ...
                          onClick={() => navigate(`/menu/dish/${recDish._id}`)}
                        >
                          {recDish.imageUrl ? (
            ...
            <div className={`hidden lg:block mt-8 pt-8 border-t border-gray-100 ${!dish.available ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <button
                  onClick={handleAddToOrder}
                  className="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-bold text-lg py-4 px-8 rounded-xl shadow-xl shadow-amber-200 hover:shadow-2xl transition-all transform hover:-translate-y-1 flex items-center justify-center gap-3"
                >
                  <span>Add to Order</span>
                  <span className="bg-amber-700/50 px-2 py-0.5 rounded text-sm">₹{dish.price * quantity}</span>
                </button>
